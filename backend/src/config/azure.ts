...
const azureConfig: AzureConfig = {
  ...
  templatesContainerName:
    process.env.AZURE_TEMPLATES_CONTAINER_NAME || "certificate-templates",
...
export class AzureBlobService {
  ...
  private templatesContainerName: string;
  ...
  async uploadCertificate(
    ...
    try {
      ...
      if (shouldCompress) {
        console.log(
          `🗜️ Compressing certificate file (${fileBuffer.length} bytes)...`
        );
        finalBuffer = await gzipAsync(fileBuffer);
        finalFileName = fileName + ".gz";
        contentEncoding = "gzip";
        console.log(
          `✅ Compressed: ${fileBuffer.length} → ${
            finalBuffer.length
          } bytes (${Math.round(
            (1 - finalBuffer.length / fileBuffer.length) * 100
          )}% reduction)`
        );
      }
  ...
  async downloadCertificate(fileName: string): Promise<Buffer> {
    try {
      ...
      const isCompressed =
        fileName.endsWith(".gz") || contentEncoding === "gzip";
  ...
  async uploadTemplate(
    ...
    try {
      ...
      const blockBlobClient =
        containerClient.getBlockBlobClient(templateFileName);
   ...
   */
  async downloadTemplate(fileName: string): Promise<Buffer> {
    try {
      const containerClient = this.blobServiceClient.getContainerClient(
        this.templatesContainerName
      );
   ...
   */
  async deleteTemplate(fileName: string): Promise<boolean> {
    try {
      const containerClient = this.blobServiceClient.getContainerClient(
        this.templatesContainerName
      );
   ...
   */
  generateTemplateFileName(templateId: string, originalName: string): string {
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  ...
  async generateSASUrl(
    fileName: string,
    expiryHours: number = 8760,
    containerType: "certificates" | "templates" = "certificates"
  ): Promise<string> {
    try {
      if (!azureConfig.accountName || !azureConfig.accountKey) {
        console.warn(
          "⚠️ Missing Azure account credentials for SAS generation, returning basic URL"
        );
      ...
      console.log(
        `✅ Generated SAS URL for ${containerType} valid for ${expiryHours} hours`
      );
      return sasUrl;
    } catch (error) {
   ...
   */
  async getSecureTemplateUrl(fileName: string): Promise<string> {
   ...
   */
  async getSecureCertificateUrl(fileName: string): Promise<string> {
