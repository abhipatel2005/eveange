...
export class TemplateService {
   ...
   */
  static async uploadTemplateToAzure(
    templateId: string,
    filePath: string,
    originalFileName: string
  ): Promise<{ azureUrl: string; azureFileName: string }> {
    try {
      ...
      const azureFileName = azureBlobService.generateTemplateFileName(
        templateId,
        originalFileName
      );

      // Upload to Azure
      const azureUrl = await azureBlobService.uploadTemplate(
        azureFileName,
        fileBuffer,
        templateId
      );
      ...
      const secureUrl = await azureBlobService.getSecureTemplateUrl(
        azureFileName
      );
   ...
   */
  static async downloadTemplateFromAzure(
   ...
   */
  static async migrateTemplateToAzure(templateId: string): Promise<boolean> {
    try {
      ...
      // Upload to Azure
      const azureUrl = await this.uploadTemplateToAzure(
        templateId,
        localFilePath,
      ...
      if (updateError) {
        throw new Error(
          `Failed to update template record: ${updateError.message}`
        );
      }
      ...
      console.error(
        `❌ Error migrating template ${templateId} to Azure:`,
        error
      );
      return false;
    }
   ...
   */
  static async createTemplateWithAzure(
    eventId: string,
    templateName: string,
    filePath: string,
    originalFileName: string,
    ...
    try {
      ...
      const { azureUrl, azureFileName } = await this.uploadTemplateToAzure(
        templateId,
        filePath,
        originalFileName
      );
   ...
   */
  static async getTemplateFile(templateId: string): Promise<Buffer> {
    try {
      // Get template data
      const { data: template, error } = await supabase
        .from("certificate_templates")
      ...
      if (template.uses_azure_storage && template.azure_url) {
        ...
        const azureFileName =
          template.template?.azure_file_name ||
          azureBlobService.generateTemplateFileName(
            templateId,
            template.template.file_name
          );
  ...
  static async deleteTemplate(templateId: string): Promise<boolean> {
    try {
      ...
      if (deleteError) {
        throw new Error(
          `Failed to delete template from database: ${deleteError.message}`
        );
      }
   ...
   */
  static async migrateAllTemplatesToAzure(): Promise<{
    success: number;
    failed: number;
    skipped: number;
  }> {
    try {
      ...
      console.log(
        `✅ Migration completed: ${success} success, ${failed} failed, ${skipped} skipped`
      );
