...
// Available data fields from database that can be mapped to certificate placeholders
export interface CertificateDataField {
  key: string;
  label: string;
  description: string;
...
export const AVAILABLE_DATA_FIELDS: CertificateDataField[] = [
  // Participant fields
  {
...
export interface PlaceholderMapping {
  [placeholder: string]: string; // placeholder -> dataField key
}
...
export class CertificateGenerator {
   ...
   */
  static async getEventData(eventId: string): Promise<EventData> {
    const { data, error } = await supabase
      .from("events")
      .select(
        `
   ...
   */
  static async extractPlaceholdersFromPPTX(
    filePath: string,
    templateId?: string
  ): Promise<string[]> {
    try {
      ...
      if (templateId) {
        try {
          console.log(
            `📥 Loading template ${templateId} for placeholder extraction...`
          );
          templateData = await TemplateService.getTemplateFile(templateId);
        } catch (azureError) {
          console.warn(
            `⚠️ Could not load template from storage, using local path:`,
            azureError
          );
   ...
   */
  static async generatePowerPointCertificate(
    templatePath: string,
    ...
    try {
      ...
      if (templateId) {
        try {
          ...
          console.warn(
            `⚠️ Could not load template from storage, falling back to local path:`,
            azureError
          );
      ...
      for (const [filename, file] of Object.entries(contents.files)) {
        if (
          ...
          Object.entries(placeholderMapping).forEach(
            ([placeholder, dataFieldKey]) => {
              const value =
                certificateData[dataFieldKey] || `{{${placeholder}}}`;
      ...
      for (const element of template.elements) {
        switch (element.type) {
          case "text":
            ...
            const text =
              certificateData[element.dataField] || element.text || "";
  ...
  static async saveCertificate(
    ...
    try {
      ...
      console.log(
        `🔍 Verifying registration ${registrationId} exists for event ${eventId}...`
      );
      ...
      if (regError || !registration) {
        ...
        throw new Error(
          `Registration ${registrationId} not found for event ${eventId}`
        );
      }

      console.log(
        `✅ Registration verified: ${registration.name} (${registration.email})`
      );
      ...
      const fileName = azureBlobService.generateFileName(
        eventId,
        registrationId,
        fileExtension
      );
      ...
      console.log(
        `📤 Uploading certificate to Azure Blob Storage: ${fileName}`
      );

      // Upload to Azure Blob Storage
      const azureUrl = await azureBlobService.uploadCertificate(
        fileName,
        certificateBuffer
      );
      ...
      console.log(
        `✅ Certificate record saved to database with ID: ${data.id}`
      );
  ...
  static async emailCertificate(
    ...
    try {
      ...
      const success = await EmailService.sendCertificateEmail(
        participantEmail,
        participantName,
        eventTitle,
        certificateUrl,
      ...
      console.log(
        `✅ Certificate email sent successfully to ${participantEmail}`
      );
    } catch (error) {
   ...
   */
  private static async updateEmailSentStatus(
    certificateCode: string,
    emailSent: boolean,
    sentByUserId?: string
  ): Promise<void> {
    try {
      ...
      if (error) {
        ...
        console.log(
          `📝 Updated email_sent to '${emailSent}' for certificate ${certificateCode}`
        );
        if (emailSent) {
          console.log(
            `📝 Email sent timestamp recorded: ${updateData.email_sent_at}`
          );
        }
      }
    } catch (error) {
