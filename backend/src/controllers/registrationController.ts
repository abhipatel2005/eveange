...
import { z } from "zod";
import { EmailService, EmailTemplateData } from "../services/emailService.js";
import { getUserGmailToken, getFreshAccessToken } from "../routes/emailAuth.js";
...
export class RegistrationController {
  ...
  static async registerForEvent(
    ...
    try {
      ...
      // Use null for free events since "not_required" is not a valid enum value
      const paymentStatus = event.is_paid ? "pending" : null;
      ...
      if (!event.is_paid) {
        console.log(
          "📧 Attempting to send registration confirmation email to:",
          userEmail
        );
        ...
        try {
          ...
          if (emailSent) {
            console.log(
              `✅ Registration confirmation sent successfully to: ${userEmail}`
            );
          } else {
            console.log(
              `⚠️ Registration confirmation could not be sent to: ${userEmail} (but registration was created)`
            );
          }
        } catch (emailError) {
        ...
        console.log(
          "💳 Paid event - confirmation email will be sent after payment completion"
        );
      }
  ...
  static async getRegistrationById(req: AuthenticatedRequest, res: Response) {
    try {
      const { registrationId } = req.params;
      const userId = req.user!.id;
