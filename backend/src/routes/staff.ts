...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const combinedEvents = assignments
        .map((assignment: any) => {
          ...
          return {
            ...
            permissions: {
              can_check_in:
                assignment.permissions?.includes("check-in") || false,
              can_view_stats:
                assignment.permissions?.includes("view-stats") || false,
            },
      ...
      console.log(
        `✅ Found ${combinedEvents.length} assigned events for staff user:`,
        userId
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      console.log(
        "📋 Fetching event details for staff user:",
        userId,
        "event:",
        eventId
      );
      ...
      if (assignmentError || !assignment) {
        console.error(
          "❌ Staff not assigned to event or event not found:",
          assignmentError
        );
      ...
      console.log(
        "📊 Single assignment query result:",
