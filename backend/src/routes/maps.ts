...
interface OlaMapsSearchResponse {
  predictions?: Array<{
    place_id: string;
    description: string;
...
interface MapMyIndiaSearchResponse {
  ...
  try {
    ...
    if (olaMapsApiKey && olaMapsApiKey !== "your_ola_maps_api_key_here") {
      try {
        ...
        console.log(
          "Ola API Key (first 10 chars):",
          olaMapsApiKey.substring(0, 10) + "..."
        );
        ...
        const olaMapsUrl = `https://api.olamaps.io/places/v1/autocomplete?input=${encodeURIComponent(
          query
        ...
        if (olaMapsResponse.ok) {
          const olaMapsData: OlaMapsSearchResponse =
            await olaMapsResponse.json();
          ...
          if (transformedResults.length > 0) {
            console.log(
              `Found ${transformedResults.length} results from Ola Maps`
            );
            return res.json({
              success: true,
              data: {
          ...
          console.log(
            `Ola Maps API failed with status: ${olaMapsResponse.status}`
          );
        ...
        console.log(
          "Ola Maps API error, falling back to MapMyIndia:",
          olaMapsError
        );
      }
    } else {
    ...
    if (
      ...
      try {
        ...
        console.log(
          "MapMyIndia API Key (first 10 chars):",
          mapMyIndiaApiKey.substring(0, 10) + "..."
        );
        ...
        mapMyIndiaUrl = `https://search.mappls.com/search/places/autosuggest/json?query=${encodeURIComponent(
          query
        )}&region=${region}&access_token=${mapMyIndiaApiKey}`;

        console.log(
          "MapMyIndia request URL (correct endpoint):",
          mapMyIndiaUrl
        );
        ...
        if (mapMyIndiaResponse.ok) {
          const mapMyIndiaData: MapMyIndiaSearchResponse =
            await mapMyIndiaResponse.json();
          ...
          if (
            mapMyIndiaData.suggestedLocations &&
            mapMyIndiaData.suggestedLocations.length > 0
          ) {
            transformedResults = mapMyIndiaData.suggestedLocations.map(
              (place) => ({
          ...
          if (transformedResults.length > 0) {
            console.log(
              `Found ${transformedResults.length} results from MapMyIndia`
            );
            return res.json({
              success: true,
              data: {
          ...
          console.log(
            `MapMyIndia API failed with status: ${mapMyIndiaResponse.status}`
          );
          ...
          if (mapMyIndiaResponse.status === 412) {
            ...
            try {
              ...
              const altUrl = `https://atlas.mapmyindia.com/api/places/search/json?query=${encodeURIComponent(
                query
              )}&region=${region}&access_token=${mapMyIndiaApiKey}`;
              ...
              if (altResponse.ok) {
                ...
                let transformedResults: any[] = [];
                if (
                  altData.suggestedLocations &&
                  altData.suggestedLocations.length > 0
                ) {
                  transformedResults = altData.suggestedLocations.map(
                    (place: any) => ({
                      name: place.placeName,
                      formatted_address: place.placeAddress,
                      geometry: {
                ...
                if (transformedResults.length > 0) {
                  console.log(
                    `Found ${transformedResults.length} results from alternative MapMyIndia endpoint`
                  );
                  return res.json({
                    success: true,
                    data: {
                ...
                console.log(
                  `Alternative MapMyIndia endpoint also failed with status: ${altResponse.status}`
                );
    ...
    res.status(500).json({
      success: false,
      error:
        "Location search services are currently unavailable. Please try again later.",
    });
  } catch (error) {
...
router.get("/reverse", async (req: Request, res: Response) => {
  try {
    ...
    if (olaMapsApiKey && olaMapsApiKey !== "your_ola_maps_api_key_here") {
      try {
        ...
        if (olaMapsResponse.ok) {
          ...
          console.log(
            `Ola Maps reverse geocoding failed with status: ${olaMapsResponse.status}`
          );
        }
      } catch (olaMapsError) {
        console.log(
          "Ola Maps reverse geocoding error, falling back to MapMyIndia:",
          olaMapsError
        );
      }
    } else {
      console.log(
        "Ola Maps API key not configured, trying MapMyIndia for reverse geocoding"
      );
    }
    ...
    if (
      ...
      try {
        ...
        if (mapMyIndiaResponse.ok) {
          ...
          console.log(
            `MapMyIndia reverse geocoding failed with status: ${mapMyIndiaResponse.status}`
          );
        }
      } catch (mapMyIndiaError) {
        console.log(
          "MapMyIndia reverse geocoding error, falling back to OpenStreetMap:",
          mapMyIndiaError
        );
      }
    } else {
      console.log(
        "MapMyIndia API key not configured, using OpenStreetMap for reverse geocoding"
      );
    }
    ...
    res.status(500).json({
      success: false,
      error:
        "Reverse geocoding services are currently unavailable. Please try again later.",
    });
  } catch (error) {
