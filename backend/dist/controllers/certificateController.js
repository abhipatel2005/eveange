...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (allowedTypes.includes(ext)) {
            ...
            cb(new Error("Only PowerPoint (.pptx) and image files (.png, .jpg, .jpeg) are allowed"));
        }
    },
});
export const certificateController = {
    // Get available data fields for certificate mapping
    getAvailableDataFields: async (req, res) => {
        try {
            res.json({
                success: true,
        ...
        async (req, res) => {
            try {
                ...
                if (type === "powerpoint" &&
                    ...
                    placeholders = await CertificateGenerator.extractPlaceholdersFromPPTX(filePath);
                }
                else if (type === "canvas") {
    ...
    updatePlaceholderMapping: async (req, res) => {
        try {
    ...
    generateCertificates: async (req, res) => {
        try {
            ...
            const unmappedPlaceholders = mappingEntries.filter(([, dataField]) => !dataField);
            ...
            // Get participants
            const allParticipants = await CertificateGenerator.getEligibleParticipants(eventId);
            ...
            for (const participant of participants) {
                try {
                    ...
                    const certificateCode = CertificateGenerator.generateCertificateCode();
                    const verificationCode = CertificateGenerator.generateVerificationCode();
                    // Generate certificate data
                    const certificateData = CertificateGenerator.generateCertificateData(participant, eventData, certificateCode, serialNumber++);
                    let certificateBuffer;
                    let fileExtension;
                    if (template.type === "powerpoint" &&
                        ...
                        const isPdf = certificateBuffer.subarray(0, 4).toString() === "%PDF";
                        fileExtension = isPdf ? "pdf" : "pptx";
                        console.log(`📄 Certificate generated as ${fileExtension.toUpperCase()}`);
                    }
                    else {
                        console.log(`🎯 Generating Canvas certificate using template ${templateId}`);
                        certificateBuffer =
                            await CertificateGenerator.generateCanvasCertificate(certificateData, template.template_data);
                        fileExtension = "png";
                    }
                    // Save certificate
                    const certificateUrl = await CertificateGenerator.saveCertificate(participant.id, eventId, templateId, certificateBuffer, certificateCode, verificationCode, fileExtension);
                    results.push({
            ...
            const responseData = {
                ...
                message: `Generated ${successCount} certificates successfully${errorCount > 0 ? `, ${errorCount} failed` : ""}`,
            };
    ...
    emailCertificates: async (req, res) => {
        try {
            const { eventId } = req.params;
        ...
        `)
            ...
            for (const certificate of certificates) {
                try {
                    ...
                    console.log(`✅ Certificate emailed to ${certificate.registrations.email}`);
                }
                catch (error) {
                    console.error(`❌ Failed to email certificate to ${certificate.registrations.email}:`, error);
                    emailResults.push({
                        certificateId: certificate.id,
                        participantName: certificate.registrations.name,
                        participantEmail: certificate.registrations.email,
            ...
            res.json({
                ...
                message: `Emailed ${successCount} certificates successfully${failedCount > 0 ? `, ${failedCount} failed` : ""}`,
            });
        }
        catch (error) {
        ...
        `)
            ...
            try {
                ...
                if (data.uses_azure_storage && data.azure_file_name) {
                    ...
                    console.log(`📥 Downloading certificate from Azure: ${data.azure_file_name}`);
                    const { azureBlobService } = await import("../config/azure.js");
                    certificateBuffer = await azureBlobService.downloadCertificate(data.azure_file_name);
                    ...
                    console.log(`📁 Reading certificate from local path: ${data.certificate_url}`);
                ...
                if (data.certificate_url) {
                    res.json({
                        ...
                        data: {
                            ...
                            message: "Direct download link (certificate file not accessible from server)",
                        },
                    });
                }
                else {
