...
const createCertificateImage = async (participantName, eventTitle, eventDate, certificateCode) => {
    const canvas = createCanvas(CERTIFICATE_CONFIG.width, CERTIFICATE_CONFIG.height);
    ...
    ctx.lineWidth = 8;
    ctx.strokeRect(20, 20, CERTIFICATE_CONFIG.width - 40, CERTIFICATE_CONFIG.height - 40);
    // Inner border
    ctx.strokeStyle = CERTIFICATE_CONFIG.titleColor;
    ctx.lineWidth = 2;
    ctx.strokeRect(40, 40, CERTIFICATE_CONFIG.width - 80, CERTIFICATE_CONFIG.height - 80);
    // Title
    ctx.fillStyle = CERTIFICATE_CONFIG.titleColor;
    ctx.font = CERTIFICATE_CONFIG.fonts.title;
    ctx.textAlign = "center";
    ctx.fillText("CERTIFICATE OF PARTICIPATION", CERTIFICATE_CONFIG.width / 2, 150);
    // Subtitle
    ...
    ctx.fillStyle = CERTIFICATE_CONFIG.textColor;
    ctx.fillText("has successfully participated in", CERTIFICATE_CONFIG.width / 2, 360);
    // Event title
    ctx.font = CERTIFICATE_CONFIG.fonts.subtitle;
    ...
    ctx.font = "16px Arial";
    ctx.textAlign = "right";
    ctx.fillText(`Certificate Code: ${certificateCode}`, CERTIFICATE_CONFIG.width - 60, CERTIFICATE_CONFIG.height - 60);
    // Date of issue
    ctx.textAlign = "left";
    ctx.fillText(`Issued on: ${new Date().toLocaleDateString()}`, 60, CERTIFICATE_CONFIG.height - 60);
    return canvas.toBuffer("image/png");
};
// Create PowerPoint certificate from template
const createPowerPointCertificate = async (templatePath, placeholderMapping, certificateData) => {
    try {
...
const generateCertificateData = (participant, event, certificateCode) => {
    return {
...
router.post("/generate", authenticateToken, async (req, res) => {
    try {
        ...
        if (event.organizer_id !== userId) {
            return res.status(403).json({
                error: "Access denied. Only event organizers can generate certificates.",
            });
        }
        // Check if event has ended
        const now = new Date();
            ...
            try {
                ...
                if (templateId && templateId !== "default") {
                    ...
                    if (template.template?.type === "powerpoint") {
                        ...
                        const certificateData = generateCertificateData(registration, event, certificateCode);
                        certificateBuffer = await createPowerPointCertificate(template.template.file_path, template.template.placeholder_mapping || {}, certificateData);
                        fileExtension = "pptx";
                    }
                    else {
                        // Use canvas for custom or default template
                        certificateBuffer = await createCertificateImage(registration.name, event.title, eventDate, certificateCode);
                    }
                }
                else {
                ...
                const { data: certificate, error: certError } = await supabase
                    ...
                    .single();
                if (certError) {
                    errors.push(`Failed to save certificate for ${registration.name}: ${certError.message}`);
                    continue;
                }
                ...
                errors.push(`Failed to generate certificate for participant: ${errorMessage}`);
            }
        }
        res.json({
            success: true,
