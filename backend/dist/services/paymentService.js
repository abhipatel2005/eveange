...
export class PaymentService {
    ...
    static async createPaymentIntent(paymentData) {
        ...
        `)
            ...
            console.log(`💰 Payment amount conversion: $${amount} -> ${amountInCents} cents`);
        ...
        try {
            ...
            if (registrationId) {
                ...
                try {
                    ...
                    console.error("Failed to send webhook confirmation email:", emailError);
    ...
    static async sendPaymentConfirmationEmail(registrationId) {
        ...
        `)
            ...
            const emailSent = await EmailService.sendRegistrationConfirmation(registration.email, emailData, freshTokenData?.email, freshTokenData?.accessToken, freshTokenData?.refreshToken);
            if (emailSent) {
                console.log(`✅ Payment confirmation email sent successfully to: ${registration.email}`);
            }
            else {
                console.log(`⚠️ Payment confirmation email could not be sent to: ${registration.email}`);
            }
        }
        catch (error) {
