...
<body>
    ...
    static async sendStaffAssignmentNotification(email, name, eventTitle, loginUrl) {
    ...
    `;
        try {
            ...
            console.log(`✅ Staff assignment notification sent successfully via SMTP to ${email}`);
            return true;
        }
        catch (smtpError) {
            console.warn("⚠️ SMTP email sending failed, trying Gmail API as fallback:", smtpError);
            // Try Gmail API as fallback
            if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
                try {
                    await sendEmailViaGmailAPI(process.env.GMAIL_USER, process.env.GMAIL_APP_PASSWORD, "", email, subject, html);
                    console.log(`✅ Staff assignment notification sent successfully via Gmail API to ${email}`);
                    return true;
                }
                catch (gmailError) {
