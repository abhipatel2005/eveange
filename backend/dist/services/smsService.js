...
class SMSService {
    ...
    initializeTwilio() {
        ...
        if (accountSid && authToken && phoneNumber) {
            ...
            console.warn("⚠️ Twilio not configured. Add TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_PHONE_NUMBER to .env");
            this.isConfigured = false;
        }
    }
    /**
     ...
     */
    async generatePhoneVerification(userId, phoneNumber) {
        try {
    ...
    async sendOTP(phoneNumber, otpCode, userName) {
        ...
        try {
            ...
            if (process.env.TWILIO_MESSAGING_SERVICE_SID) {
                ...
                const errorMsg = "Either TWILIO_MESSAGING_SERVICE_SID or TWILIO_PHONE_NUMBER must be configured.\n" +
                    "🔧 To fix this:\n" +
     ...
     */
    async sendPhoneVerificationOTP(userId, phoneNumber, userName) {
        try {
            // Generate OTP and store in database
            const generateResult = await this.generatePhoneVerification(userId, phoneNumber);
            ...
            // Send SMS
            const sendResult = await this.sendOTP(phoneNumber, generateResult.otpCode, userName);
    ...
    async resendOTP(phoneNumber) {
        try {
            ...
            return await this.sendPhoneVerificationOTP(lastVerification[0].user_id, phoneNumber);
        }
        catch (error) {
